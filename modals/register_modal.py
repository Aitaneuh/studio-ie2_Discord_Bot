...
class RegisterModal(discord.ui.Modal, title="Enregistrement Clash Royale"):
    def __init__(self):
        ...
        self.first_name = discord.ui.TextInput(label="Prénom", max_length=30)
        self.last_name = discord.ui.TextInput(label="Nom", max_length=30)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if player_role in interaction.user.roles:
            return await interaction.response.send_message(
                embed=get_simple_embed("Vous êtes déjà enregistré.", color=discord.Color.red()),
                ephemeral=True,
            )
        ...
        if player_data is None:
            return await interaction.followup.send(
                embed=get_simple_embed("Veuillez entrer un TAG valide.", color=discord.Color.red()),
                ephemeral=True,
            )
        ...
        try:
            ...
            try:
                ...
                pass  # Pas de permission pour changer le pseudo (ex : owner du serveur)

            embed = get_simple_embed(
                title="Enregistrement complété",
                description=(
                    f"Merci {first_name} {last_name}, votre compte a bien été enregistré !\n\n"
                    f"**Pseudo :** {pseudo}\n"
                    f"**Tag :** {tag}\n"
                    f"**Trophées :** {trophies}"
                ),
